...
export const api = new Hono()
  ...
  .get('/config', async (c) =>{
    ...
    const protocol = url.protocol === 'https' || req.header('x-forwarded-proto') === 'https' ? 'wss' : 'ws';
  ...
  .get('/projects/:projectName/sessions/:sessionId/messages', async (c) => {
    ...
    try {
      ...
      if (limit !== null) {
        ...
        console.log(`Getting messages ${startIndex} to ${endIndex} of ${totalMessages}`);
        console.log(`Are there more messages? ${startIndex > 0}`);
