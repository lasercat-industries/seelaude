      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && !isExpanded) {
            ...
            const details = messageRef.current.querySelectorAll('details[data-auto-expand="true"]');
            ...
            <details className="group">
              <summary className="cursor-pointer list-none">
                <div className="flex items-center gap-2">
                  <svg className="w-4 h-4 transition-transform group-open:rotate-90 text-gray-600 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        return (
                          <details className="mt-2" open={autoExpandTools} data-auto-expand="true">
                            ...
                            <div className="mt-3">
                              ...
                              {showRawParameters && (
                                <details className="mt-3" open={autoExpandTools} data-auto-expand="true">
                                  <summary className="text-xs text-blue-600 dark:text-blue-400 cursor-pointer hover:text-blue-700 dark:hover:text-blue-300">
                                    View raw parameters
                                  </summary>
                            ...
                            <div className="mt-3 space-y-3">
                              {input.edits.map((edit, index) => (
                                <div key={index} className="border border-gray-200 dark:border-gray-700 rounded-lg p-3">
                                  <div className="text-xs text-gray-600 dark:text-gray-400 mb-2">Edit {index + 1} of {editCount}</div>
                                  <div className="grid grid-cols-1 gap-2">
                                    <div>
                                      <div className="text-xs text-red-600 dark:text-red-400 mb-1">- Old</div>
    ...
    for (const msg of rawMessages) {
      ...
      if (msg.message?.role === 'user' && msg.message?.content) {
        ...
        if (Array.isArray(msg.message.content)) {
          ...
          if (msg.message.content.trim().startsWith('[{') && msg.message.content.includes('"tool_use_id"') && msg.message.content.includes('"type":"tool_result"')) {
            try {
                ...
                for (const toolResult of parsed) {
                  if (toolResult.type === 'tool_result') {
                    converted.push({
                      ...
                      isError: toolResult.is_error || (typeof toolResult.content === 'string' && toolResult.content.includes('<tool_use_error>')) || false
                    });
                  }
                }
          ...
          for (const part of msg.message.content) {
            if (part.type === 'tool_result') {
              converted.push({
                ...
                isError: part.is_error || (typeof part.content === 'string' && part.content.includes('<tool_use_error>')) || false
              });
            }
          }
        }
